...
#include <string>
#include "rtmp/rtmp.h"
#include "XLive/safe_queue.h"
#include "XLive/PushInterface.h"
#include "XLive/VideoStream.h"
#include "XLive/AudioStream.h"
...
Live_FUNC(void, native_1setVideoCodecInfo,
...
Live_FUNC(void, native_1start, jstring path_) {
    LOGI("native start...");
    if (isStart) {
        return;
    }
...
Live_FUNC(void, native_1pushVideo, jbyteArray data_) {
...
Live_FUNC(void, native_1pushVideoNew, jbyteArray y, jbyteArray u, jbyteArray v) {
    if (!videoStream || !readyPushing) {
        return;
    }
...
Live_FUNC(void, native_1setAudioCodecInfo, jint sampleRateInHz, jint channels) {
    if (audioStream) {
        audioStream->setAudioEncInfo(sampleRateInHz, channels);
    }
}

Live_FUNC(jint, getInputSamples) {
    if (audioStream) {
...
Live_FUNC(void, native_1pushAudio, jbyteArray data_) {
...
Live_FUNC(void, native_1release) {
    LOGI("native release...");
