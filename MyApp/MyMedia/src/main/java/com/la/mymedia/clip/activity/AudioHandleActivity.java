...
import com.la.mymedia.player.AudioPlayer;
import com.la.mymedia.clip.handler.FFmpegHandler;
import com.la.mymedia.player.mp3.Mp3Converter;
...
public class AudioHandleActivity extends BaseActivity {
    ...
    private void doHandleAudio(final String srcFile) {
        ...
        } else if (viewId == R.id.btn_cut) {//cut audio, it's best not include special characters
        ...
        } else if (viewId == R.id.btn_concat) {//concat audio
...
//                break;
            concatAudio(srcFile);
            return;
        } else if (viewId == R.id.btn_mix) {//mix audio
            if (!FileUtil.checkFileExist(appendFile)) {
                return;
            }
        ...
        } else if (viewId == R.id.btn_play_audio) {//use AudioTrack to play audio
            new Thread(() -> new AudioPlayer().play(srcFile)).start();
            return;
        } else if (viewId == R.id.btn_play_opensl) {//use OpenSL ES to play audio
            new Thread(() -> new AudioPlayer().playAudio(srcFile)).start();
            return;
        } else if (viewId == R.id.btn_audio_encode) {//audio encode
        ...
        } else if (viewId == R.id.btn_pcm_concat) {//concat PCM streams
