...
import com.la.mymedia.clip.FFmpegCmd;
import com.la.mymedia.clip.format.VideoLayout;
import com.la.mymedia.clip.handler.FFmpegHandler;
import com.la.mymedia.clip.model.MediaBean;
import com.la.mymedia.clip.util.JsonParseTool;
import com.la.mymedia.util.BitmapUtil;
import com.la.mymedia.clip.util.FFmpegUtil;
import com.la.mymedia.util.FileUtil;
...
import static com.la.mymedia.clip.handler.FFmpegHandler.MSG_BEGIN;
import static com.la.mymedia.clip.handler.FFmpegHandler.MSG_FINISH;
import static com.la.mymedia.clip.handler.FFmpegHandler.MSG_PROGRESS;
...
public class VideoHandleActivity extends BaseActivity {
    ...
    private void doHandleVideo(String srcFile) {
        ...
        if (viewId == R.id.btn_video_transform) {//transform format
            ...
            int startTime = 0;
            int duration = 200;
            Log.e("ddddddd", cutVideo);

            commandLine = FFmpegUtil.cutVideo(srcFile, startTime, duration, cutVideo);
            Log.e("ddddddd", cutVideo);
        } else if (viewId == R.id.btn_video_concat) {//concat video together
            concatVideo(srcFile);
        } else if (viewId == R.id.btn_screen_shot) {//video snapshot
            String screenShot = PATH + File.separator + "screenShot.jpg";
            int time = 18;
            commandLine = FFmpegUtil.screenShot(srcFile, time, screenShot);
        } else if (viewId == R.id.btn_water_mark) {//add watermark to video
            ...
            switch (waterMarkType) {
                case TYPE_IMAGE:// image
                    ...
                    commandLine = FFmpegUtil.addWaterMarkImg(VideoHandleActivity.this, srcFile, photo, location, bitRate, offsetXY, photoMark);
                    break;
                case TYPE_GIF:// gif
                    String gifPath = PATH + File.separator + "ok.gif";
                    String gifWaterMark = PATH + File.separator + "gifWaterMark.mp4";
                    commandLine = FFmpegUtil.addWaterMarkGif(VideoHandleActivity.this, srcFile, gifPath, location, bitRate, offsetXY, gifWaterMark);
                    break;
                case TYPE_TEXT:// text
                    ...
                    commandLine = FFmpegUtil.addWaterMarkImg(VideoHandleActivity.this, srcFile, textPath, location, bitRate, offsetXY, textMark);
                    break;
                default:
                    break;
            }
        } else if (viewId == R.id.btn_generate_gif) {//convert video into gif
        ...
        } else if (viewId == R.id.btn_screen_record) {//screen recording
...
//                commandLine = FFmpegUtil.screenRecord(screenSize, recordTime, screenRecord);
        } else if (viewId == R.id.btn_multi_video) {//combine video which layout could be horizontal of vertical
        ...
        } else if (viewId == R.id.btn_pip) {//combine into picture-in-picture video
        ...
        } else if (viewId == R.id.btn_moov) {//moov box moves ahead, which is behind mdat box of mp4 video
        ...
        } else if (viewId == R.id.btn_speed) {//playing speed of video
