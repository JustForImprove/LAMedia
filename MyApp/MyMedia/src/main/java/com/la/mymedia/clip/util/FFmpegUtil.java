package com.la.mymedia.clip.util;

import android.annotation.SuppressLint;
import android.content.Context;


import com.la.mymedia.util.ScreenUtil;
...
//        String transformVideoCmd = "ffmpeg -i %s -r 25 -b 200 -s 1080x720 %s";
     ...
     */
    public static String[] addWaterMarkImg(Context context, String srcFile, String imgPath, int location, int bitRate, int offsetXY, String targetFile) {
        String mBitRate = bitRate + "k";
        String overlay;
        int offset = ScreenUtil.dp2px(context, offsetXY);
        if (location == 1) {
     ...
     * @return add watermark success or not
     */
    public static String[] addWaterMarkGif(Context context, String srcFile, String imgPath, int location, int bitRate, int offsetXY, String targetFile) {
        String mBitRate = bitRate + "k";
        String overlay;
        int offset = ScreenUtil.dp2px(context, offsetXY);
        if (location == 1) {
...
//                "\"[0:v]pad=iw*2:ih*2[a];[a][1:v]overlay=w[b];[b][2:v]overlay=0:h[c];[c][3:v]overlay=w:h\" %s";
        ...
        if (videoLayout == LAYOUT_VERTICAL) {//vstack: vertical
